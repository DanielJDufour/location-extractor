...
from .converter import get_text_from_pdf_file, get_text_and_tables_from_docx_file
from .enums import data_dir, keywords_dir, letters_dir
from .utils import flatten, isJavaScript, read_lines

flags = MULTILINE|UNICODE

languages = listdir(keywords_dir)
...
def load_non_locations():
    global nonlocations
    with open(join(data_dir, "nonlocations.txt")) as f:
        for line in f:
            if line and not line.startswith("#"):
                nonlocations.add(line.strip())

def load_language_into_dictionary_of_keywords(language):
    global dictionary_of_keywords
    dictionary_of_keywords[language] = {}
    directory_of_language_files = join(keywords_dir, language)
    ...
    for filename in language_files:
        ...
        if not filename.startswith("."):
            ...
            if filename == "abbreviations.tsv":
                with open(directory_of_language_files + "/" + filename) as f:
                    dictionary_of_keywords[language]['abbreviations'] = dict(reader(f, delimiter="\t"))

            elif filename == "demonyms.tsv":
                with open(directory_of_language_files + "/" + filename) as f:
                    dictionary_of_keywords[language]['demonyms'] = dict(reader(f, delimiter="\t"))

            elif filename == "cities.txt":
                with open(directory_of_language_files + "/" + filename) as f:
                    dictionary_of_keywords[language]['cities'] = f.read().strip().split("\n")

            elif filename == "countries.tsv":
                with open(directory_of_language_files + "/" + filename) as f:
                    dictionary_of_keywords[language]['countries'] = dict(reader(f, delimiter="\t"))
...
def load_language_into_dictionary_of_letters(language):
    global dictionary_of_letters
    letters = set()
    with open(join(letters_dir, language + ".txt")) as f:
...
def extract_locations_from_text(text, debug=False, return_demonyms=False, return_abbreviations=False, return_country_info=False, case_insensitive=None):
    ...
    if debug: print("str([text])[:500]:", str([text])[:500])
    ...
    for language in languages:
        ...
        if language == "Chinese":
            ...
            langdict = dictionary_of_keywords[language]
            cities = langdict['cities']

            for unigram in unigrams:
                if unigram in cities:
                    match = search(seq, text)
                    locations.add({ "start": match.start(), "end": match.end(), "location": unigram })
            for n in range(2, 10):
                for gram in ngrams(unigrams, n):
                    seq = ''.join(gram)
                    if seq in cities:
                        match = search(seq, text)
                        results.append({ "start": match.start(), "end": match.end(), "location": seq, "city": seq })
