...
class FOSS4G2018(TestCase):
    def test_circle(self):
        ...
        locations = extract_locations_with_context_from_text(
            source, suggestions, debug=False
        )
    ...
    def test_circle2(self):
        ...
        locations = extract_locations_with_context_from_text(
            source, suggestions, debug=False, return_abbreviations=return_abbreviations
        )
...
class FOSS4G2017(TestCase):
    def test_cambridge(self):
        ...
        locations = extract_locations_with_context_from_text(
            source, suggestions=suggestions, debug=False
        )
        ...
        self.assertEqual(
            next(
                ...
                if location["name"] == "Cambridge" and location["admin1code"] == "MA"
            ),
            1,
        )
...
class TestArabic(TestCase):
    def test_demonyms(self):
        ...
        locations = extract_locations_with_context(
            source, suggestions=[source], debug=False
        )
        self.assertEqual(locations[0]["name"], u"\u0642\u0637\u0631")
...
class TestMethods(unittest.TestCase):
    ...
    def test_max_seconds(self):
        with open(path_to_directory_of_this_file + "/performance.txt") as f:
            ...
            with timeout(seconds=10):
                locations = extract_locations_with_context(
                    text, debug=False, max_seconds=5
                )
            seconds_took = (datetime.now() - start).total_seconds()
    ...
    def test_counting(self):
        ...
        locations = extract_locations_with_context(text)
        location = [
            location for location in locations if location["name"] == "Germany"
        ][0]
...
القمة العربية في موريتانيا تدعو لتكريس الجهود لحل القضية الفلسطينية"""
    ...
    def test_state_abbreviations_with_context(self):
        ...
        try:
            ...
            print(
                "extract_locations_with_context's args:",
                getargspec(extract_locations_with_context),
            )
            print("locations:", locations)
            print(e)
            raise e
    ...
    def test_abbreviations_with_context(self):
        ...
        try:
            names = [u"Arlington"]
            locations = extract_locations_with_context(
                text, names, debug=False, return_abbreviations=True
            )
            self.assertEqual(len(locations), 1)
    ...
    def testBugFix(self):
        locations = extract_locations_with_context(
            "Alexandria, VA", debug=False, return_abbreviations=True
        )
        self.assertEqual(len(locations), 1)
